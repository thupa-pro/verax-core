...
pub fn run() -> Result<()> {
    ...
    println!(
        "  \u{2502}  {:^53}\u{2502}",
        "Verax Protocol — Interactive Tutorial"
    );
    ...
    step(
        ...
        "We'll create a simple provenance statement.",
    )?;
    wait();

    let tmp = std::env::temp_dir().join("verax-tutorial");
    std::fs::create_dir_all(&tmp)?;
    let artifact = tmp.join("hello.txt");
    std::fs::write(&artifact, b"Hello, Verax!")?;

    let artifact_hash = verax_core::hash::blake3(b"Hello, Verax!");
    ...
    let payload = verax_core::VeraxPayload::new(artifact_hash, verax_core::Predicate::Attests);
    ...
    step(
        ...
        "Full protocol verification — checks signature, canonical CBOR, and key binding.",
    )?;
    wait();

    match verax_core::verify_statement_ed25519(stmt.to_bytes(), &vk)
        .map_err(|e| anyhow::anyhow!("verify failed: {}", e))
    {
        Ok(_) => println!(
            "  \u{2714} VERIFIED — Signature, canonical CBOR, and key binding all valid.\n"
        ),
    ...
    step(
        5,
        "Building a provenance chain",
        "Statements can link to form a chain.",
    )?;
    wait();

    let stmt_hash = verax_core::hash::blake3(stmt.to_bytes());
    let mut child_payload =
        verax_core::VeraxPayload::new(artifact_hash, verax_core::Predicate::Appends);
    ...
    println!(
        "  Statement 1: {} -> Statement 2 (APPENDS)",
        &hex::encode(stmt_hash)[..16]
    );
    println!();
    ...
    step(
        6,
        "Understanding trust",
        "Trust comes from the key, not the chain.",
    )?;
    wait();
    ...
    println!("  so `verax verify` works with zero flags.\n");

    println!("  \u{250C}{}\u{2510}", "\u{2500}".repeat(55));
    println!("  \u{2502}  {:^53}\u{2502}", "Tutorial Complete!");
    println!("  \u{2514}{}\u{2518}\n", "\u{2500}".repeat(55));
    println!("  What you learned:");
    println!("    verax init        — create a project");
    println!("    verax sign <file> — sign a statement (auto-generates key)");
    println!("    verax verify <file> — verify a statement (auto-extracts key)");
    println!("    verax graph <file> — visualize the provenance chain");
    println!("    verax lint <file> — check for best practices\n");
    println!("  Try:  verax examples image\n");
