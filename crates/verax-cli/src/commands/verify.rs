...
use verax_core::{
    CompositePublicKey, Statement, TrustStore, VeraxPayload, VerificationWarnings, Warning, cose,
    hash::blake3, verify_statement_with_warnings,
};
...
impl TrustStore for CliTrustStore {
    ...
    fn resolve_mldsa65_key(&self, _kid: &[u8]) -> Option<ml_dsa::VerifyingKey<ml_dsa::MlDsa65>> {
        ...
        let raw =
            ml_dsa::EncodedVerifyingKey::<ml_dsa::MlDsa65>::try_from(&comp_pk.mldsa65[..]).ok()?;
    ...
    match result {
        Ok((stmt, warnings)) => {
            ...
            if use_json {
                ...
                let algorithm = if is_composite {
                    "Ed25519 + ML-DSA-65 (Hybrid)"
                } else {
                    "Ed25519"
                };
                let trust_level = if is_composite {
                    "HIGH (PQ-secure)"
                } else {
                    "STANDARD"
                };
                ...
                let mut sections =
                    build_sections(args, &Some(ed_vk), &payload, algorithm, trust_level);
                let warn_sections = build_warning_sections(&warnings);
                sections.extend(warn_sections);

                if args.explain {
                    let explain_sections = generate_explain_sections(
                        args,
                        &Some(ed_vk),
                        algorithm,
                        &data,
                ...
                let report = Report {
                    title: "Verax Protocol — Verification Report".into(),
                    sections,
                    overall: Some(if has_warnings {
                        Verdict::Partial
                    } else {
                        Verdict::Verified
                    }),
        ...
        Err(e) => {
            ...
            if use_json {
                ...
                let report = Report {
                    ...
                    sections: vec![
                        Section {
                            label: args.file.file_name().unwrap().to_string_lossy().into(),
                            status: Status::Info,
                            detail: None,
                            indent: 0,
                        },
                        Section {
                            label: "Protocol check".into(),
                            status: Status::Fail,
                            detail: Some(msg),
                            indent: 1,
                        },
...
fn build_trust_store(
    ...
    let ed_vk = if let Some(pk_hex) = &args.pubkey {
        ...
        if k.len() != 32 {
            anyhow::bail!(
                "expected 32-byte KID (Ed25519 pubkey), got {} bytes",
                k.len()
            );
        }
        let mut arr = [0u8; 32];
        ...
        anyhow::bail!(
            "no public key provided and no KID found in COSE envelope.\n\
             ...
             `verax sign` which embeds the public key automatically."
        );
    };
    ...
    let (is_composite, ml_bytes, comp_pk) = if let Some(ml_hex) = &args.mldsa_pubkey {
        ...
        if ml_bytes.len() != 1952 {
            anyhow::bail!(
                "ML-DSA-65 pubkey must be 1952 bytes (got {})",
                ml_bytes.len()
            );
        }
        let mut arr = [0u8; 1952];
        arr.copy_from_slice(&ml_bytes);
        let comp = CompositePublicKey {
        ...
        match w {
            Warning::TemporalEvidenceMissing => {
                sections.push(Section {
                    ...
                    status: Status::Warn,
                    detail: Some(
                        "statement has no CT log anchor; timestamp not provable to third parties"
                            .into(),
                    ),
                    indent: 1,
                });
            ...
            Warning::RevocationStatusUnknown => {
                sections.push(Section {
                    ...
                    status: Status::Warn,
                    detail: Some(
                        "cannot verify whether statement has been revoked (no CT log access)"
                            .into(),
                    ),
                    indent: 1,
                });
            ...
            Warning::StaleSth {
                ...
                sections.push(Section {
                    ...
                    status: Status::Warn,
                    detail: Some(format!(
                        "STH from {} is {}s after statement from {}",
                        sth_time, delta, stmt_time
                    )),
                    indent: 1,
                });
            }
...
fn build_json_output(
    ...
    if let Some(p) = payload {
        ...
        if let Some(o) = p.object {
            obj["object"] = serde_json::json!(hex::encode(o));
        }
        if let Some(ts) = p.timestamp {
            obj["timestamp"] = serde_json::json!(ts);
        }
        if let Some(n) = p.nonce {
            obj["nonce"] = serde_json::json!(hex::encode(n));
        }
        if let Some(l) = p.lineage {
            obj["lineage_hash"] = serde_json::json!(hex::encode(l));
        }
        if let Some(ah) = p.anchor_hash {
            obj["anchor_hash"] = serde_json::json!(hex::encode(ah));
        }
    }
    obj
}
...
fn generate_explain_sections(
    ...
    let mut sections = vec![
        Section {
            label: "Step 1: Loaded COSE envelope from file".into(),
            status: Status::Pass,
            detail: Some(format!("{} bytes", data.len())),
            indent: 1,
        },
        Section {
            label: "Step 2: Verified canonical CBOR encoding".into(),
            status: Status::Pass,
            detail: None,
            indent: 1,
        },
        Section {
            label: "Step 3: Verified protected header determinism".into(),
            status: Status::Pass,
            detail: None,
            indent: 1,
        },
    ...
    if let Some(vk) = vk {
        sections.push(Section {
            label: format!(
                "Step 4: Resolved public key from {} header",
        ...
        sections.push(Section {
            label: "Step 4: Resolved composite public key".into(),
            status: Status::Pass,
            detail: None,
            indent: 1,
        });
    ...
    sections.push(Section {
        label: format!("Step 5: Verified {} signature", algorithm),
        status: Status::Pass,
        detail: None,
        indent: 1,
    });
    ...
    if let Some(l) = stmt.lineage().ok().flatten() {
        sections.push(Section {
            label: "Step 6: Verified lineage chain integrity".into(),
            status: Status::Pass,
            detail: Some(format!("previous: {}", hex::encode(l))),
            indent: 1,
        });
    }
    ...
    let has_anchor = !warnings
        .warnings
        .iter()
        .any(|w| matches!(w, Warning::TemporalEvidenceMissing));
    if has_anchor {
        sections.push(Section {
            label: "Step 7: Verified CT temporal anchor".into(),
            status: Status::Pass,
            detail: Some("Merkle proof and STH signature valid".into()),
            indent: 1,
        });
    } else {
        sections.push(Section {
            label: "Step 7: CT temporal anchor".into(),
            status: Status::Warn,
            detail: Some("not present; timestamp not provable".into()),
            indent: 1,
        });
    }

    let revoked_unknown = warnings
        .warnings
        .iter()
        .any(|w| matches!(w, Warning::RevocationStatusUnknown));
    if revoked_unknown {
        sections.push(Section {
            label: "Step 8: Revocation check".into(),
            status: Status::Warn,
            detail: Some("CT log not reachable; revocation status unknown".into()),
            indent: 1,
        });
    } else {
        sections.push(Section {
    ...
    if let Some(ph) = payload_hash {
        sections.push(Section {
            label: format!("Payload hash: {}", hex::encode(ph)),
            status: Status::Info,
            detail: None,
            indent: 1,
        });
    ...
    sections.push(Section {
        label: format!("Statement hash: {}", hex::encode(stmt_hash)),
        status: Status::Info,
        detail: None,
        indent: 1,
    });
...
fn generate_trace_sections(data: &[u8], payload_bytes: &Option<Vec<u8>>) -> Vec<Section> {
    let mut sections = Vec::new();
    sections.push(Section {
        label: "Raw Statement Bytes".into(),
        status: Status::Info,
        detail: Some(hex::encode(data)),
        indent: 1,
    });
    if let Some(pb) = payload_bytes {
        sections.push(Section {
            ...
            status: Status::Info,
            detail: Some(hex::encode(pb)),
            indent: 1,
        });
        sections.push(Section {
            label: "Payload Hash (BLAKE3)".into(),
            status: Status::Info,
            detail: Some(hex::encode(blake3(pb))),
            indent: 1,
        });
    }
    sections.push(Section {
        ...
        status: Status::Info,
        detail: Some(hex::encode(blake3(data))),
        indent: 1,
    });
    sections
}
