...
pub fn run(args: &ExportArgs) -> Result<()> {
    ...
    let payload_bytes = verax_core::cose::extract_payload(&data)
        .map_err(|e| anyhow::anyhow!("failed to parse: {}", e))?;
    let payload = verax_core::VeraxPayload::decode(&payload_bytes)
        .map_err(|e| anyhow::anyhow!("failed to decode: {}", e))?;

    let hash = verax_core::hash::blake3(&data);

    match args.export_format.as_str() {
        "markdown" | "md" => {
            let pred_str = format!("{:?}", payload.predicate);
            let object_str = payload
                .object
                .map(|o| format!("`{}`", hex::encode(o)))
                .unwrap_or_else(|| "_none_".into());
            let ts_str = payload
                .timestamp
                .map(crate::output::format_timestamp)
                .unwrap_or_else(|| "_none_".into());
            let nonce_str = payload
                .nonce
                .map(|n| format!("`{}`", hex::encode(n)))
                .unwrap_or_else(|| "_none_".into());
            let lineage_str = payload
                .lineage
                .map(|l| format!("`{}`", hex::encode(l)))
                .unwrap_or_else(|| "_none_".into());
            let md = format!(
                r#"# Verax Statement
...
"#,
        ...
        "svg" | "pdf" => {
            anyhow::bail!(
                "{} export not yet implemented — try 'markdown' or 'json'",
                args.export_format
            );
        }
        other => anyhow::bail!(
            "unsupported export format: '{}' (use: markdown, json)",
            other
        ),
    }
