...
pub fn run(args: &ConsentArgs) -> Result<()> {
    ...
    let policy_hash = verax_core::hash::blake3(b"consent-policy-default-v1");

    let consent_payload = verax_core::shred::create_consent_payload(
        policy_hash,
        verax_core::hash::blake3(&data).as_slice(),
        ts,
        args.purpose.as_bytes(),
    );

    let subject_hash = verax_core::hash::blake3(&consent_payload.encode());
    let mut sign_payload =
        verax_core::VeraxPayload::new(subject_hash, verax_core::Predicate::CompliesWith);
    sign_payload.object = Some(policy_hash);
    sign_payload.timestamp = Some(ts);

    let stmt = verax_core::Statement::sign_ed25519(&sign_payload, &sk)
