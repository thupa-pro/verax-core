...
use verax_core::{Predicate, Statement, cose, hash::blake3};
...
pub fn run(args: &ExplainArgs) -> Result<()> {
    ...
    match payload.predicate {
        ...
        Predicate::DerivedFrom => {
            println!(
                "  declares that artifact `{}` was DERIVED FROM",
                subject_short
            );
        ...
        Predicate::Appends => {
            println!(
                "  APPENDS new data to lineage of artifact `{}`",
                subject_short
            );
        }
        Predicate::Endorses => {
    ...
    if let Some(lineage) = payload.lineage {
        println!(
            "  This statement extends lineage from `{}`",
            hex::encode(lineage)
        );
    }
