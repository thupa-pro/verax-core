...
use verax_core::{Predicate, VeraxPayload, cose};
...
pub fn run(args: &LintArgs) -> Result<()> {
    ...
    let payload_bytes =
        cose::extract_payload(&data).map_err(|e| anyhow::anyhow!("failed to parse COSE: {}", e))?;
    ...
    if payload.timestamp.is_none() {
        warnings.push(
            "missing timestamp — statements should include a timestamp for temporal ordering"
                .into(),
        );
    }
    ...
    if payload.predicate == Predicate::Revokes && payload.object.is_none() {
        warnings
            .push("REVOKES without object — should reference the revoked statement hash".into());
    }
    ...
    if payload.nonce.is_some() && payload.timestamp.is_none() {
        warnings.push(
            "nonce present but no timestamp — nonce is only meaningful with repeated timestamps"
                .into(),
        );
    }
        ...
        && !exts.is_empty()
    {
        warnings.push(format!(
            "{} custom extension(s) — ensure these are interoperable",
            exts.len()
        ));
    }
    ...
    if payload_bytes.len() > 1024 {
        warnings.push(format!(
            "payload is {} bytes — consider keeping payloads under 1KB",
            payload_bytes.len()
        ));
    }
    ...
    if is_ed25519 {
        best_practices.push(
            "Ed25519 signature — consider composite (Ed25519 + ML-DSA-65) for quantum resistance"
                .into(),
        );
    }
    ...
    if args.json {
        ...
        let report = Report {
            ...
            sections,
            overall: Some(if warnings.is_empty() {
                Verdict::Verified
            } else {
                Verdict::Partial
            }),
