...
fn create(args: &DidCreateArgs) -> Result<()> {
    ...
    let doc_path = config::project_dir()
        .join("trust")
        .join(format!("{}.did.json", name));
    ...
    let report = Report {
        ...
        sections: vec![
            ...
            Section {
                label: "Key File".into(),
                status: Status::Info,
                detail: Some(key_path.to_string_lossy().into()),
                indent: 0,
            },
            Section {
                label: "DID Document".into(),
                status: Status::Info,
                detail: Some(doc_path.to_string_lossy().into()),
                indent: 0,
            },
        ],
...
fn resolve(args: &DidResolveArgs) -> Result<()> {
    ...
    let report = Report {
        ...
        sections: vec![
            ...
            Section {
                label: "Raw Key Bytes".into(),
                status: Status::Info,
                detail: Some(format!("{} bytes", decoded.1.len())),
                indent: 0,
            },
            Section {
                label: "Hex".into(),
                status: Status::Info,
                detail: Some(hex::encode(&decoded.1)),
                indent: 0,
            },
        ],
        overall: if decoded.1.len() == 32 {
...
fn verify(args: &DidVerifyArgs) -> Result<()> {
    ...
    let result = verax_core::cose::parse_and_verify_ed25519(&data, &vk);
