...
fn submit(args: &LogSubmitArgs) -> Result<()> {
    ...
    let stmt_hash = verax_core::hash::blake3(&data);
    ...
    std::fs::write(
        cache_dir.join(format!("{}.axm", hex::encode(stmt_hash))),
        &data,
    )?;
    ...
    let report = Report {
        ...
        sections: vec![
            ...
            Section {
                label: "Log".into(),
                status: Status::Info,
                detail: Some("local (simulated)".into()),
                indent: 0,
            },
            Section {
                label: "Temporal Anchor".into(),
                status: Status::Warn,
                detail: Some("no anchor present in COSE".into()),
                indent: 1,
            },
        ],
    ...
    let report = Report {
        ...
        sections: vec![
            Section {
                label: "Hash".into(),
                status: Status::Info,
                detail: Some(args.hash.clone()),
                indent: 0,
            },
            Section {
                label: "Cached Locally".into(),
                status: if cached { Status::Pass } else { Status::Warn },
                detail: None,
                indent: 0,
            },
            Section {
                label: "Log Submission".into(),
                status: Status::Warn,
                detail: Some("no remote log configured".into()),
                indent: 0,
            },
        ],
        overall: Some(if cached {
            Verdict::Verified
        } else {
            Verdict::Partial
        }),
...
fn inclusion(args: &LogInclusionArgs) -> Result<()> {
    ...
    let report = Report {
        ...
        sections: vec![
            ...
            Section {
                label: "Proof Type".into(),
                status: Status::Info,
                detail: Some("Merkle inclusion (simulated)".into()),
                indent: 0,
            },
            Section {
                label: "Status".into(),
                status: Status::Warn,
                detail: Some("simulated — no remote CT log".into()),
                indent: 0,
            },
        ],
        overall: Some(Verdict::Partial),
