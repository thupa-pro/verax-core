...
use verax_core::{Statement, VeraxPayload, hash::blake3, predicate::Predicate};
...
pub fn run(args: &GraphArgs) -> Result<()> {
    ...
    let stmt =
        Statement::from_bytes(&data).map_err(|e| anyhow::anyhow!("failed to parse COSE: {}", e))?;
    let payload = stmt
        .decode_payload()
            ...
            Some(prev_hash) => {
                ...
                chain.push(StatementNode {
                    ...
                    payload: prev_payload
                        .clone()
                        .unwrap_or_else(|| VeraxPayload::new([0; 32], Predicate::Attests)),
                    depth,
                });
...
fn print_ascii_graph(chain: &[StatementNode]) {
    ...
    for node in chain.iter().rev() {
        ...
        let styled = Style::new()
            .bold()
            .paint(format!("  {} [{}]", label, pred_str));
        println!("{}", styled);
        println!(
            "  {}    subject: {}",
            " ".repeat(depth_offset),
            &hex::encode(node.payload.subject)[..12]
        );
        ...
        if let Some(ts) = node.payload.timestamp {
            println!(
                "  {}    time: {}",
                " ".repeat(depth_offset),
                crate::output::format_timestamp(ts)
            );
        }
...
fn fetch_statement(hash: &[u8; 32]) -> Option<Vec<u8>> {
    ...
    let paths = [
        format!(".verax/cache/{}.axm", hash_hex),
        format!("{}.axm", hash_hex),
    ];
    for p in &paths {
