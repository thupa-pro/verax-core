...
static PROJECT_CONFIG: OnceLock<VeraxConfig> = OnceLock::new();
...
pub fn load_config() -> VeraxConfig {
...
fn load_config_from_disk() -> VeraxConfig {
    let path = project_config_path();
    if !path.exists() {
        return VeraxConfig::default();
    }
    let content = match std::fs::read_to_string(&path) {
        Ok(c) => c,
        Err(_) => return VeraxConfig::default(),
    };
