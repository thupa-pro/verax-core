...
fn generate(args: &KeyGenerateArgs) -> Result<()> {
    ...
    let out_path = args.out.clone().unwrap_or_else(|| config::project_keys_dir().join(format!("{}.key", name)));

    let key_content = format!(
        "# Axiom Ed25519 signing key\n# name: {name}\n# public: {}\n{}\n",
    ...
    let report = Report {
        ...
        sections: vec![
            Section { label: "Name".into(), status: Status::Info, detail: Some(name), indent: 0 },
            Section { label: "Algorithm".into(), status: Status::Info, detail: Some("Ed25519".into()), indent: 0 },
            Section { label: "Public Key".into(), status: Status::Info, detail: Some(hex::encode(vk.to_bytes())), indent: 0 },
            Section { label: "Saved".into(), status: Status::Pass, detail: Some(out_path.to_string_lossy().into()), indent: 0 },
        ],
...
fn export_key(args: &KeyExportArgs) -> Result<()> {
    ...
    let data = std::fs::read_to_string(&key_path)
        .map_err(|_| anyhow::anyhow!("key '{}' not found in .axiom/keys/", args.name))?;
...
fn rotate(args: &KeyRotateArgs) -> Result<()> {
    ...
    let current_hex = current_data.lines()
        .find(|l| !l.starts_with('#') && !l.trim().is_empty())
    ...
    let payload = axiom_core::AxiomPayload {
        subject: current_vk.to_bytes(),
        predicate: axiom_core::Predicate::Supersedes,
    ...
    let stmt = axiom_core::Statement::sign_ed25519(&payload, &current_sk)
        .map_err(|e| anyhow::anyhow!("signing failed: {}", e))?;

    let out_path = args.out.clone()
        .unwrap_or_else(|| config::project_keys_dir().join(format!("{}.to.{}.axm", args.current_key_name, args.new_key_name)));
    ...
    println!("  Rotated key: {} \u{2192} {}", args.current_key_name, args.new_key_name);
...
fn list() -> Result<()> {
    ...
    if !keys_dir.exists() {
        println!("  No keys found (run `axiom init` first)");
        return Ok(());
    }
    ...
    if entries.is_empty() {
        println!("  No keys found in .axiom/keys/");
        return Ok(());
    }

    println!("\n  Keys in .axiom/keys/:\n");
    for (name, pubkey, _path) in &entries {
        let short = if pubkey.len() > 16 { &pubkey[..16] } else { pubkey };
