...
use axiom_core::predicate::{Predicate, CORE_PREDICATES};
...
fn parse_predicate(s: &str) -> Result<Predicate> {
    ...
    anyhow::bail!("unknown predicate: '{}' (use: attests, authors, derived_from, supersedes, revokes, appends, endorses, complies_with)", s)
}
...
pub fn run(args: &SignArgs) -> Result<()> {
    ...
    let (cose_bytes, algorithm, pubkey_hex, ml_pubkey_hex) = if has_anchor {
        ...
        let mut offset = 0;
        let inclusion_proof = axiom_core::LogInclusionProof::from_cbor(&anchor_data, &mut offset)
            .map_err(|e| anyhow::anyhow!("failed to decode LogInclusionProof: {e}"))?;
        let log_sth = axiom_core::SignedTreeHead::from_cbor(&anchor_data, &mut offset)
            .map_err(|e| anyhow::anyhow!("failed to decode SignedTreeHead: {e}"))?;
        let anchor = axiom_core::TemporalAnchor { inclusion_proof, signed_tree_head: log_sth };
        ...
        if is_composite {
            ...
            let comp_pk = axiom_core::composite_pubkey(&ed_vk, &ml_vk);

            (cose, "Ed25519 + ML-DSA-65 (Hybrid) + CT Anchor", hex::encode(comp_pk.ed25519), Some(hex::encode(comp_pk.mldsa65)))
        } else {
            ...
            (stmt.to_bytes().to_vec(), "Ed25519 + CT Anchor", hex::encode(vk.to_bytes()), None)
        }
        ...
        (cose, "Ed25519 + ML-DSA-65 (Hybrid)", hex::encode(comp_pk.ed25519), Some(hex::encode(comp_pk.mldsa65)))
    } else {
        let sk_bytes = read_signing_key(args)?;
        let sk = ed25519_dalek::SigningKey::from_bytes(&sk_bytes);
        let vk = sk.verifying_key();
        ...
        (stmt.to_bytes().to_vec(), "Ed25519", hex::encode(vk.to_bytes()), None)
    };

    let out_path = args.out.clone().unwrap_or_else(|| {
        let mut p = args.file.clone();
        let ext = p.extension().map(|e| format!(".{}.axm", e.to_string_lossy()))
    ...
    if args.json {
        ...
        use crate::output::{Status, Section, Report, Verdict};
        let mut sections = vec![
            Section { label: "Output".into(), status: Status::Pass, detail: Some(out_path.to_string_lossy().into()), indent: 0 },
            Section { label: "Subject (BLAKE3)".into(), status: Status::Info, detail: Some(hex::encode(subject)), indent: 0 },
            Section { label: "Predicate".into(), status: Status::Info, detail: Some(predicate.name().into()), indent: 0 },
            Section { label: "Algorithm".into(), status: Status::Info, detail: Some(algorithm.into()), indent: 0 },
            Section { label: "Ed25519 Public Key".into(), status: Status::Info, detail: Some(pubkey_hex.clone()), indent: 0 },
        ];
        if let Some(ml) = &ml_pubkey_hex {
            sections.push(Section { label: "ML-DSA-65 Public Key".into(), status: Status::Info, detail: Some(ml.clone()), indent: 0 });
        }
        sections.push(Section { label: "Statement Hash".into(), status: Status::Info, detail: Some(hex::encode(stmt_hash)), indent: 0 });
        let report = Report {
...
fn read_mldsa_key(args: &SignArgs) -> Result<ml_dsa::SigningKey<ml_dsa::MlDsa65>> {
    let seed = if let Some(key_path) = &args.ml_dsa_key {
        ...
        let hex_str = hex_line.trim().strip_prefix("0x").unwrap_or(hex_line.trim());
...
fn read_signing_key(args: &SignArgs) -> Result<[u8; 32]> {
    ...
    let key_content = format!(
        "# Axiom Ed25519 signing key (auto-generated)\n# name: {name}\n# public: {}\n{}\n",
        hex::encode(vk.to_bytes()),
        hex::encode(seed)
    );
    std::fs::write(&out_path, key_content.as_bytes())?;
    eprintln!("  Generated ephemeral key: {} (public: {})", name, hex::encode(vk.to_bytes()));
