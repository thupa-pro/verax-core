...
pub fn run() -> Result<()> {
    ...
    let payload = axiom_core::AxiomPayload::new([0xab; 32], axiom_core::Predicate::Attests);
    let bytes = payload.encode();
    let decoded = axiom_core::AxiomPayload::decode(&bytes).unwrap();
    ...
    let h1 = axiom_core::hash::blake3(b"axiom-test");
    let h2 = axiom_core::hash::blake3(b"axiom-test");
    if h1 == h2 && h1 != [0u8; 32] {
    ...
    if axiom_core::cose::parse_and_verify_ed25519(stmt.to_bytes(), &vk).is_ok() {
    ...
    if axiom_core::cose::parse_and_verify_ed25519(stmt.to_bytes(), &bad_vk).is_err() {
    ...
    if axiom_core::cose::parse_and_verify_ed25519(&tampered, &vk).is_err() {
    ...
    let mut full_payload = axiom_core::AxiomPayload::new([0x01; 32], axiom_core::Predicate::DerivedFrom);
    ...
    let full_decoded = axiom_core::AxiomPayload::decode(&full_bytes).unwrap();
    ...
    let parent_payload = axiom_core::AxiomPayload::new([0xaa; 32], axiom_core::Predicate::Attests);
    let parent_stmt = axiom_core::Statement::sign_ed25519(&parent_payload, &sk)
        .map_err(|e| anyhow::anyhow!("sign failed: {}", e))?;
    let parent_hash = axiom_core::hash::blake3(parent_stmt.to_bytes());
    let mut child_payload = axiom_core::AxiomPayload::new([0xaa; 32], axiom_core::Predicate::Appends);
    child_payload.lineage = Some(parent_hash);
    let child_stmt = axiom_core::Statement::sign_ed25519(&child_payload, &sk)
        .map_err(|e| anyhow::anyhow!("sign failed: {}", e))?;
    if axiom_core::cose::parse_and_verify_ed25519(child_stmt.to_bytes(), &vk).is_ok() {
    ...
    // Total
    println!("\n  Results: {} passed, {} failed, {} total", passed, failed, passed + failed);
    if failed > 0 {
