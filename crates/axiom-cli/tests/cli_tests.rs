...
fn test_cli_version() {
    ...
    assert!(output.status.success(), "version failed: {}", String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("axiom"), "version output missing 'axiom': {stdout}");
}
...
fn test_cli_help() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Commands"), "help output missing 'Commands': {stdout}");
}
...
fn test_cli_sign_verify() {
    ...
    std::fs::write(&artifact, b"hello axiom").unwrap();
    ...
    assert!(out.status.success() || stderr.contains("PASS"), "verify failed: {}", stderr);
}
...
fn test_cli_init() {
    ...
    assert!(dir.path().join(".axiom").exists(), ".axiom dir not created");
    assert!(dir.path().join(".axiom/config.toml").exists(), "config.toml not created");
}
...
fn test_cli_hash() {
    ...
    let out = cli()
        ...
        .output()
        .unwrap();
    assert!(out.status.success(), "hash failed: {}", String::from_utf8_lossy(&out.stderr));
