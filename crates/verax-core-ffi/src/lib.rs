...
use verax_core::cose::CompositePublicKey;
use verax_core::cose::MLDSA65_PK_SIZE;
use verax_core::cose::VerificationMode;
use verax_core::cose::parse_and_verify_composite;
use verax_core::cose::parse_and_verify_ed25519;
use verax_core::cose::parse_and_verify_mldsa65_only;
use verax_core::error::Error;
use verax_core::{
    Predicate, ShreddingKey, Statement, TrustStore, VeraxPayload, VerificationWarnings, Warning,
    decrypt_pii, encrypt_pii, hash::blake3, shredding_commit, verify_statement_with_warnings,
};

fn pk_from_slice(bytes: &[u8]) -> Result<VerifyingKey, Error> {
    let arr: [u8; 32] = bytes
        .try_into()
        .map_err(|_| Error::InvalidField("pubkey must be exactly 32 bytes for Ed25519"))?;
...
/// Section B of the Verax Protocol specification.
///
/// | Code | Name                     |
/// |------|--------------------------|
/// |  1   | MalformedCose            |
...
fn error_to_code(e: &Error) -> i32 {
    match e {
        ...
        // triggers this because VeraxPayload::encode() is infallible, but
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_version() -> *const core::ffi::c_char {
    static VERSION: &str = concat!("verax-core ", env!("CARGO_PKG_VERSION"));
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_verify_mldsa65_only(
    cose_data: *const u8,
    cose_len: usize,
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_verify_ed25519(
    cose_data: *const u8,
    cose_len: usize,
    pubkey_data: *const u8,
    pubkey_len: usize,
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_verify_composite(
    cose_data: *const u8,
    cose_len: usize,
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_free(ptr: *mut u8) {
    ffi_free(ptr);
}

/// Decodes a CBOR-encoded VeraxPayload into its component fields.
///
/// Matches the C signature specified in Section F of the Verax Protocol:
/// ```c
/// int verax_payload_decode(
...
/// ```
#[unsafe(no_mangle)]
pub extern "C" fn verax_payload_decode(
    cbor_data: *const u8,
    cbor_len: usize,
    out_subject: *mut u8,
    ...
    let payload = match VeraxPayload::decode(cbor) {
        Ok(p) => p,
        Err(e) => return error_to_code(&e),
    };
    unsafe {
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_sign_ed25519(
    ...
    let payload = match VeraxPayload::decode(payload_bytes) {
        Ok(p) => p,
        Err(e) => return error_to_code(&e),
    };
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_sign_composite(
    payload_data: *const u8,
    payload_len: usize,
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_encrypt_pii(
    key_data: *const u8,
    key_len: usize,
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_decrypt_pii(
    key_data: *const u8,
    key_len: usize,
    ct_data: *const u8,
    ct_len: usize,
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_shredding_commit(
    key_data: *const u8,
    key_len: usize,
    plaintext_data: *const u8,
    plaintext_len: usize,
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_encode_payload(
...
#[unsafe(no_mangle)]
pub extern "C" fn verax_verify_full(
    cose_data: *const u8,
    cose_len: usize,
    pubkey_data: *const u8,
    pubkey_len: usize,
    ...
    let tlk = if !trusted_log_key_data.is_null() && trusted_log_key_len == 32 {
        let mut arr = [0u8; 32];
        unsafe {
            ptr::copy_nonoverlapping(trusted_log_key_data, arr.as_mut_ptr(), 32);
        }
        Some(arr)
    } else {
        None
    ...
    let store = FfiTrustStore {
        ...
        checkpoint_timestamp: if checkpoint_timestamp > 0 {
