...
Node.js bindings for the Axiom Protocol core library.
...
```bash
npm install axiom-core-node
```

Or build from source:

```bash
cargo build --release -p axiom-core-node
```

## Usage

```javascript
const axiom = require('axiom-core-node');

// Sign
const payload = axiom.encodePayload(Buffer.alloc(32), "attests");
const key = Buffer.alloc(32);
const sig = axiom.signEd25519(payload, key);

// Verify
const result = axiom.verifyFull(sig, key, null, [], [], [], 0);
