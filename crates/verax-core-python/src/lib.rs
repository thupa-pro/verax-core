//! Python bindings for the Verax Protocol.
//!
//! This module exposes the core Verax functionality to Python:
...
//! descriptive messages and numeric error codes matching the Verax spec.
...
use verax_core::{
    ...
    VerificationWarnings, Warning, cose, decrypt_pii, encrypt_pii, hash::blake3, shredding_commit,
...
fn payload_to_dict(py: Python, payload: &VeraxPayload) -> PyResult<PyObject> {
...
/// A decoded Verax payload with fields for subject, predicate, object,
...
fn verify_ed25519(cose: &[u8], pubkey: &[u8]) -> PyResult<Vec<u8>> {
    let arr: [u8; 32] = pubkey
        .try_into()
        .map_err(|_| pyo3::exceptions::PyValueError::new_err("pubkey must be exactly 32 bytes"))?;
...
fn verify_composite(cose: &[u8], ed_pubkey: &[u8], ml_dsa_pubkey: &[u8]) -> PyResult<Vec<u8>> {
    ...
    cose::parse_and_verify_composite(cose, &comp_pk, verax_core::cose::VerificationMode::Hybrid)
        .map_err(py_err)
}
...
/// Decodes a CBOR-encoded Verax payload into a Payload object.
///
/// Args:
...
fn decode_payload(cbor: &[u8]) -> PyResult<Payload> {
    let p = VeraxPayload::decode(cbor).map_err(py_err)?;
    Ok(Payload {
...
fn sign_ed25519(payload_cbor: &[u8], key_bytes: &[u8]) -> PyResult<Vec<u8>> {
    ...
    let payload = VeraxPayload::decode(payload_cbor).map_err(py_err)?;
...
fn sign_composite(
    ...
    let ed_arr: [u8; 32] = ed_key_bytes
        .try_into()
        .map_err(|_| pyo3::exceptions::PyValueError::new_err("ed_key must be exactly 32 bytes"))?;
    let ml_arr: [u8; 32] = ml_seed_bytes
        .try_into()
        .map_err(|_| pyo3::exceptions::PyValueError::new_err("ml_seed must be exactly 32 bytes"))?;
...
#[pymodule]
fn verax_core_python(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(verax_version, m)?)?;
