...
pub(crate) fn decode_text_string(data: &[u8], offset: &mut usize) -> Result<Vec<u8>> {
    ...
    *offset += 1;
    ...
    if major != 3 {
        return Err(Error::Decode(format!("expected text string major type, got {major}")));
    }
    ...
    let len = match info {
        ...
        _ => return Err(Error::Decode(format!("reserved additional info for text string: {info}"))),
    };
...
pub(crate) fn decode_negative_int(data: &[u8], offset: &mut usize) -> Result<i64> {
    ...
    *offset += 1;
    if major != 1 {
        return Err(Error::Decode(format!("expected negative int major type, got {major}")));
    }
    let n = match info {
        0..=23 => info as u64,
        24 => {
        ...
        _ => return Err(Error::Decode(format!("reserved additional info for negative int: {info}"))),
    };
    Ok(-1i64 - n as i64)
}
...
pub(crate) fn decode_uint(data: &[u8], offset: &mut usize) -> Result<u64> {
    ...
    *offset += 1;
    ...
    if major != 0 {
        return Err(Error::Decode(format!("expected uint major type, got {major}")));
    }
    ...
    match info {
        ...
        _ => Err(Error::Decode(format!("reserved additional info for uint: {info}"))),
    }
}
...
fn decode_bstr_len(data: &[u8], offset: &mut usize) -> Result<u64> {
    ...
    *offset += 1;
    ...
    if major != 2 {
        return Err(Error::Decode(format!("expected bstr major type, got {major}")));
    }
    ...
    let len = match info {
        ...
        _ => return Err(Error::Decode(format!("reserved additional info for bstr: {info}"))),
    };
    Ok(len)
}
...
pub(crate) fn decode_map_len(data: &[u8], offset: &mut usize) -> Result<u64> {
    ...
    *offset += 1;
    ...
    if major != 5 {
        return Err(Error::Decode(format!("expected map major type, got {major}")));
    }
    ...
    let len = match info {
        ...
        _ => return Err(Error::Decode(format!("reserved additional info for map: {info}"))),
    };
    Ok(len)
}
...
pub fn decode_payload(data: &[u8]) -> Result<AxiomPayload> {
    ...
    let subject = subject.ok_or_else(|| Error::Payload("missing required field: subject".into()))?;
    let predicate = predicate.ok_or_else(|| Error::Payload("missing required field: predicate".into()))?;
...
pub(crate) fn decode_array_len(data: &[u8], offset: &mut usize) -> Result<u64> {
    ...
    *offset += 1;
    ...
    if major != 4 {
        return Err(Error::Decode(format!("expected array major type, got {major}")));
    }
    ...
    let len = match info {
        ...
        _ => return Err(Error::Decode(format!("reserved additional info for array: {info}"))),
    };
    Ok(len)
}
...
impl RecoveryPolicy {
    ...
    pub fn decode(data: &[u8]) -> Result<Self> {
        ...
        let guardians = guardians.ok_or_else(|| Error::Payload("recovery policy: missing guardians".into()))?;
        let threshold = threshold.ok_or_else(|| Error::Payload("recovery policy: missing threshold".into()))?;
        Ok(RecoveryPolicy { guardians, threshold, recovery_delay })
    }
}
...
///
/// Proves that for any strictly-valid AxiomPayload, the
...
mod kani_proofs {
    ...
    fn check_cbor_roundtrip_full() {
        ...
        let mut p = AxiomPayload::new(subject, Predicate::DerivedFrom);
    ...
    fn check_cbor_determinism_repeated() {
        let p = AxiomPayload::new([0xab; 32], Predicate::Attests);
        let a = p.encode();
        let b = p.encode();
        assert_eq!(a, b);
    }
    ...
    fn check_cbor_encode_never_panics() {
        let subject: [u8; 32] = kani::any();
        let p = AxiomPayload::new(subject, Predicate::Attests);
        let encoded = p.encode();
...
mod tests {
    ...
    fn test_round_trip_minimal() {
        let payload = AxiomPayload::new([0xabu8; 32], Predicate::Attests);
        let encoded = payload.encode();
        let decoded = AxiomPayload::decode(&encoded).unwrap();
    ...
    fn test_round_trip_full() {
        let mut p = AxiomPayload::new([0x01; 32], Predicate::DerivedFrom);
        p.object = Some([0x02; 32]);
        p.timestamp = Some(1700000000);
        p.lineage = Some([0x03; 32]);
        p.nonce = Some([0x04; 32]);
        p.anchor_hash = Some([0x05; 32]);
        p.extensions = Some(vec![(100, Value::Uint(42)), (101, Value::Bstr(vec![1, 2, 3]))]);
    ...
    fn test_deterministic_encoding() {
        let a = {
            let mut p = AxiomPayload::new([0x01; 32], Predicate::Authors);
            p.object = Some([0x02; 32]);
            p.encode()
        };
        let b = {
    ...
    fn test_rejects_null_bytes() {
        ...
        assert!(AxiomPayload::decode(&buf).is_err());
    }
    ...
    fn test_rejects_tags() {
        ...
        let inner = AxiomPayload::new([0x01; 32], Predicate::Attests).encode();
        data.extend_from_slice(&inner);
        assert!(AxiomPayload::decode(&data).is_err());
    }
    ...
    fn test_fuzz_decode_never_panics() {
        ...
        for _ in 0..5000 {
            ...
            let _ = AxiomPayload::decode(&bytes);
        }
    }
    ...
    fn test_fuzz_rejects_duplicate_keys() {
        ...
        for _ in 0..1000 {
            ...
            assert!(AxiomPayload::decode(&buf).is_err(), "duplicate keys");
        }
    }
    ...
    fn test_fuzz_empty_input() {
        assert!(AxiomPayload::decode(&[]).is_err());
    }
    ...
    fn test_rejects_indefinite_length() {
        ...
        assert!(AxiomPayload::decode(&[0xbf, 0xff]).is_err());
        // indefinite-length array (0x9f), then break (0xff)
        assert!(AxiomPayload::decode(&[0x9f, 0xff]).is_err());
        // indefinite-length bstr (0x5f), then break (0xff)
        assert!(AxiomPayload::decode(&[0x5f, 0xff]).is_err());
        // indefinite-length text (0x7f), then break (0xff)
        assert!(AxiomPayload::decode(&[0x7f, 0xff]).is_err());
    }
    ...
    fn test_rejects_bytewise_unsorted_keys() {
        ...
        assert!(
            AxiomPayload::decode(&buf).is_err(),
    ...
    fn test_bytewise_sorted_keys_in_encoded_output() {
        let subject = [0xabu8; 32];
        let mut p = AxiomPayload::new(subject, Predicate::Attests);
        ...
        assert!(pos_key2.is_some(), "key 2 must be present in encoded output");
        assert!(pos_key10.is_some(), "key 10 must be present in encoded output");
        assert!(
