...
use axiom_core::{
    ...
    decrypt_pii, encrypt_pii, hash_ciphertext,
    shredding_commit, ShreddingKey,
    Artifact, CompositePublicKey, Predicate, Statement, TrustStore,
    AxiomPayload,
    verify_statement, verify_statement_ed25519,
    Error,
};
...
impl TrustStore for TestTrust {
    ...
    fn resolve_log_pubkey(&self, log_id: &[u8; 32], candidate_key: &[u8; 32]) -> Option<[u8; 32]> {
        let computed = axiom_core::blake3(&self.log_key);
...
fn use_case_1_digital_fingerprints() {
    ...
    assert_eq!(artifact.hash().len(), 32, "fingerprint is 32 bytes (BLAKE3)");

    let same = Artifact::new(data);
    assert_eq!(artifact.hash(), same.hash(), "identical content → identical fingerprint");

    let different = Artifact::new(b"different content");
    assert_ne!(artifact.hash(), different.hash(), "different content → completely different fingerprint");

    assert!(artifact.verify(data), "content verified against its fingerprint");

    assert!(!artifact.verify(b"tampered"), "tampered content fails fingerprint check");

    let large: Vec<u8> = (0..10_000).map(|i| (i % 256) as u8).collect();
    let large_artifact = Artifact::new(&large);
    assert!(large_artifact.verify(&large), "BLAKE3 tree-hash handles large files");
}
...
fn use_case_2_family_tree() {
    ...
    let doc_v1 = AxiomPayload::new(blake3(b"contract_v1"), Predicate::Authors);
    ...
    let h1 = blake3(stmt_v1.to_bytes());
    {
        ...
        assert!(verify_statement(stmt_v1.to_bytes(), &store).is_ok(), "v1: Alice authored contract_v1");
    }

    let mut doc_v2 = AxiomPayload::new(blake3(b"contract_v2"), Predicate::DerivedFrom);
    ...
    let h2 = blake3(stmt_v2.to_bytes());
    {
        ...
        assert!(verify_statement(stmt_v2.to_bytes(), &store).is_ok(), "v2: Alice derived contract_v2 from v1");
    }

    let mut doc_v3 = AxiomPayload::new(blake3(b"contract_v3"), Predicate::Supersedes);
    ...
    let stmt_v3 = sign_and_anchor(&doc_v3, &alice, &log_sk);
    {
        ...
        store.chain.insert(h1, stmt_v1.to_bytes().to_vec());
        store.chain.insert(h2, stmt_v2.to_bytes().to_vec());
        store.chain.insert(blake3(stmt_v3.to_bytes()), stmt_v3.to_bytes().to_vec());
        assert!(verify_statement(stmt_v3.to_bytes(), &store).is_ok(), "v3: chain of 3 statements verified");
    }

    let mut doc_broken = AxiomPayload::new(blake3(b"orphan"), Predicate::Authors);
    ...
    assert_eq!(res, Err(Error::BrokenLineage("previous statement not found".into())));
}
...
fn use_case_3_public_logbook() {
    ...
    let payload = AxiomPayload::new(blake3(b"important_document"), Predicate::Attests);
    let stmt = sign_and_anchor(&payload, &issuer, &log_op);

    let store = TestTrust::new(&issuer, &log_op);
    assert!(verify_statement(stmt.to_bytes(), &store).is_ok(), "CT-anchored statement verifies");

    let wrong_log_op = sign_sk(0x88);
    let bad_store = TestTrust::new(&issuer, &wrong_log_op);
    assert!(verify_statement(stmt.to_bytes(), &bad_store).is_err(), "wrong log key fails STH sig check");
}
...
fn use_case_4_all_predicates() {
    ...
    for (pred, name) in [
        ...
        let mut p = AxiomPayload::new(subject, pred);
    ...
    let mut p = AxiomPayload::new(blake3(b"extension_test"), Predicate::Attests);
    p.extensions = Some(vec![
        (100, axiom_core::AxiomPayloadValue::Uint(42)),
        (101, axiom_core::AxiomPayloadValue::Bstr(vec![1, 2, 3])),
    ]);
    let stmt = sign_and_anchor(&p, &issuer, &log_sk);
    let decoded = verify_statement(stmt.to_bytes(), &store)
        ...
        .decode_payload()
        .unwrap();
    assert!(decoded.extensions.is_some(), "extensions preserved through round-trip");
    assert_eq!(decoded.extensions.as_ref().unwrap().len(), 2);

    let stream_id = blake3(b"live_stream_123");
    let chunk0 = AxiomPayload::new(stream_id, Predicate::Appends);
    let stmt0 = sign_and_anchor(&chunk0, &issuer, &log_sk);
    let h0 = blake3(stmt0.to_bytes());

    let mut chunk1 = AxiomPayload::new(stream_id, Predicate::Appends);
    ...
    assert!(verify_statement(stmt1.to_bytes(), &chain_store).is_ok(), "APPENDS chain with matching subject passes");
}
...
fn use_case_5_right_to_be_forgotten() {
    ...
    assert_eq!(commitment, hash_ciphertext(&ciphertext), "commitment = hash of ciphertext");
    ...
    assert!(decrypt_pii(&wrong_key, &ciphertext).is_err(), "wrong key cannot decrypt");

    assert!(decrypt_pii(&ShreddingKey::generate(), &ciphertext).is_err(), "fresh key also cannot decrypt");

    let encrypted_twice = encrypt_pii(&shred_key, patient_data).unwrap();
    assert_ne!(encrypted_twice, ciphertext, "same data encrypts differently each time (unique nonce)");

    let comm2 = hash_ciphertext(&ciphertext);
    assert_eq!(comm2, commitment, "hash is deterministic — ciphertext can be committed on-ledger");
}
...
fn use_case_offline_verification() {
    ...
    let payload = AxiomPayload::new(blake3(b"offline_doc"), Predicate::Authors);
...
fn use_case_quantum_ready_composite() {
    ...
    let payload = AxiomPayload::new(blake3(b"quantum_document"), Predicate::Authors);
    let stmt = Statement::sign_composite(&payload, &ed_sk, &ml_sk).unwrap();

    let ml_vk = ml_sk.verifying_key();
    let pubkey = axiom_core::composite_pubkey(&ed_sk.verifying_key(), &ml_vk);

    let log_sk = sign_sk(0x99);
    let store = TestTrust::new(&ed_sk, &log_sk);
    let result = verify_statement(stmt.to_bytes(), &store);
    assert!(result.is_err(), "Ed25519 key alone cannot verify a composite signature");
    ...
    impl TrustStore for CompositeTrust {
        fn resolve_key(&self, _: &[u8]) -> Option<ed25519_dalek::VerifyingKey> { None }
        fn resolve_composite_key(&self, _: &[u8]) -> Option<CompositePublicKey> { Some(self.key.clone()) }
        fn fetch_statement(&self, _: &[u8; 32]) -> Option<Vec<u8>> { None }
        fn is_revoked_in_log(&self, _: &[u8; 32], _: u64) -> Option<bool> { Some(false) }
        fn resolve_log_pubkey(&self, log_id: &[u8; 32], candidate_key: &[u8; 32]) -> Option<[u8; 32]> {
            let computed = axiom_core::blake3(&self.log_key);
            if &computed == log_id && &self.log_key == candidate_key { Some(self.log_key) } else { None }
        }
    ...
    assert!(verify_statement(stmt.to_bytes(), &comp_store).is_ok(), "composite key verifies the statement");
}
...
fn use_case_ct_anchored_composite() {
    ...
    let mut payload = AxiomPayload::new(blake3(b"ct_composite_doc"), Predicate::DerivedFrom);
    ...
    let store = CompAnchorStore { key: pubkey.clone(), log_key: log_key_bytes };
    assert!(
    ...
    let bad_store = CompAnchorStore { key: pubkey, log_key: wrong_log.verifying_key().to_bytes() };
    assert!(
