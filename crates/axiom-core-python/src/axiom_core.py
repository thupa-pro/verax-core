"""Python bindings for Axiom Protocol Core.

Uses ctypes to load the shared library (libaxiom_core_ffi.so) and wraps
...
class AxiomError(Exception):
    """Axiom protocol error with numeric error code."""
    ...
    def __init__(self, code: int):
        ...
        super().__init__(f"[{self.name}] Axiom error {code}")
...
class _AxiomLib:
    ...
    def _find_lib(cls):
        search_paths = [
            Path(os.environ.get("AXIOM_LIB_PATH", "")),
            Path.cwd(),
        ...
        for d in search_paths:
            so = d / "libaxiom_core_ffi.so"
            if so.exists():
                return str(so)
        raise RuntimeError(
            "libaxiom_core_ffi.so not found. "
            "Set AXIOM_LIB_PATH or run from the repo root."
        )
...
def version() -> str:
    ...
    return _AxiomLib.lib().axiom_version().decode("utf-8")
...
def verify_ed25519(
    ...
    Raises:
        AxiomError: If verification fails.
    """
    lib = _AxiomLib.lib()
...
def decode_payload(cbor_bytes: bytes) -> Payload:
    """Decode an Axiom CBOR payload into structured fields.
...
if __name__ == "__main__":
    ...
    print(f"axiom-core {version()}: {n} test vectors PASS")
