...
Python bindings for the Axiom Protocol core library.
...
```bash
pip install axiom-core-python
```

Or build from source:

```bash
cargo build --release -p axiom-core-python
cp target/release/libaxiom_core_python.so /path/to/site-packages/axiom_core_python.so
```

## Usage

```python
import axiom_core_python as axiom

# Sign
payload = axiom.encode_payload(bytes(32), "attests")
key = bytes(32)
sig = axiom.sign_ed25519(payload, key)

# Verify
result = axiom.verify_full(sig, key, trusted_log_key=bytes(32))
